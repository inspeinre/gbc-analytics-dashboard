...
// СЛОВАРЬ СТАТУСОВ (Обязательно проверь коды в RetailCRM)
const STATUS_MAP = {
...
// СЛОВАРЬ ТИПОВ ЗАКАЗА (Вернул обратно)
const ORDER_TYPES = {
...
const App = () => {
  ...
  if (loading) return <div className="flex h-screen items-center justify-center font-bold text-blue-500 animate-pulse uppercase tracking-widest">Синхронизация данных...</div>;
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
          ...
          <div className="flex gap-2">
            {(filterCity || filterStatus || sortConfig.key) && (
              <button onClick={() => { setFilterCity(null); setFilterStatus(null); setSortConfig({ key: null, direction: null }); }} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 text-red-500 rounded-xl hover:bg-red-50 font-bold shadow-sm transition-all text-xs">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={cityData} onClick={(d) => d && setFilterCity(d.activeLabel)}>
                  ...
                  <Bar dataKey="value" radius={[4, 4, 4, 4]} barSize={24}>
                    {cityData.map((e, i) => <Cell key={i} fill={filterCity === e.name ? '#3b82f6' : '#f1f5f9'} className="cursor-pointer" />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50/50 text-slate-400 text-[10px] uppercase font-black tracking-widest">
                <tr>
                  ...
                  <SortableHeader label="Тип заказа" columnKey="order_type" currentSort={sortConfig} onSort={requestSort} />
              ...
              <tbody className="divide-y divide-slate-50">
                {processedOrders.map((order) => (
                  <tr key={order.id} className="hover:bg-blue-50/20 transition-colors">
                    <td className="px-8 py-5 text-[11px] font-bold text-slate-400 whitespace-nowrap">
                      {new Date(order.created_at).toLocaleDateString('ru-RU')}
                    </td>
                    <td className="px-8 py-5 text-xs font-black text-slate-900">#{order.number}</td>
                    <td className="px-8 py-5">
                      <div className="text-xs font-bold text-slate-800">{order.first_name} {order.last_name}</div>
                      <div className="text-[10px] text-slate-400 font-bold uppercase">{order.utm_source || 'direct'}</div>
                    </td>
                    <td className="px-8 py-5">
                    ...
                    <td className="px-8 py-5 font-black text-slate-900 text-sm">
                      {Number(order.total_summ).toLocaleString()} <span className="text-[10px] text-slate-400 uppercase font-black">₸</span>
                    </td>
                    <td className="px-8 py-5 text-center">
                      <span className="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider shadow-sm border border-transparent"
...
// Исправленный компонент заголовка с ЗЕЛЕНЫМИ стрелками
const SortableHeader = ({ label, columnKey, currentSort, onSort }) => {
  const isActive = currentSort.key === columnKey;

  return (
    <th
      className={`px-8 py-6 cursor-pointer group transition-all duration-200 select-none ${isActive ? 'bg-emerald-50/50 text-emerald-700' : 'hover:bg-slate-100/50 hover:text-slate-700'}`}
      onClick={() => onSort(columnKey)}
    >
      <div className="flex items-center gap-2">
        <span>{label}</span>
        <div className="flex flex-col gap-0.5">
          {isActive && currentSort.direction === 'asc' && <ChevronUp size={16} strokeWidth={4} className="text-emerald-500" />}
          {isActive && currentSort.direction === 'desc' && <ChevronDown size={16} strokeWidth={4} className="text-emerald-500" />}
          {!isActive && <ChevronsUpDown size={14} className="opacity-10 group-hover:opacity-100 text-slate-400" />}
        </div>
      </div>
    </th>
  );
...
const KpiCard = ({ title, value, icon, color, bg }) => (
  <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 flex items-center gap-6">
    <div className={`p-5 ${bg} ${color} rounded-2xl`}>{icon}</div>
    <div>
